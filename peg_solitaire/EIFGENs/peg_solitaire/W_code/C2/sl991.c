...
extern EIF_TYPED_VALUE F991_7399(EIF_REFERENCE, EIF_TYPED_VALUE);
...
EIF_TYPED_VALUE F991_7399 (EIF_REFERENCE Current, EIF_TYPED_VALUE arg1x)
{
	GTCX
...
#define ur1 ur1x.it_r
	...
	RTEAA(l_feature_name, 990, Current, 0, 1, 13647);
	RTSA(dtype);
	RTSC;
	RTME(dtype, 0);
	RTGC;
...
body:;
	...
	if (RTAL & CK_ENSURE) {
		...
		*(EIF_BOOLEAN *)tr2 = Result;
		...
		tr4 = RTLN(eif_new_type(202, 0x00).id);
		*(EIF_BOOLEAN *)tr4 = RTEQ(tr2, tr1);
		if (RTEQ(tr4, tr3)) {
			RTCK;
		} else {
			RTCF;
